...
## ZFaktury-Specific State Patterns

### Service as State Source

In ZFaktury, business logic lives in services (zfaktury-core), not in GPUI entities. Views hold `Arc<Service>` and load state on demand:
...
pub struct InvoiceListView {
    ...
    loading: bool,
    error: Option<String>,
    filter_status: Option<InvoiceStatus>,
}
```

This differs from pure GPUI patterns where models are GPUI entities. Our services are plain Rust structs behind `Arc`, not `Entity<T>`. This means:
- No `cx.observe()` on services -- views must explicitly reload
- No `cx.emit()` from services -- use `NavigateEvent` from views
- State refresh is pull-based (view calls `load_data`), not push-based

### Navigation as Global Event

Route changes flow through `NavigateEvent`, which the root view handles:

```rust
// Any view can request navigation
cx.emit(NavigateEvent(Route::InvoiceDetail(42)));

// Root view (RootView in root.rs) subscribes to all child views
// and swaps the active view based on the route
```
