...
// optionally runs OCR within the given company. Returns the combined result.
func (s *ImportService) ImportFromDocument(ctx context.Context, companyID int64, filename, contentType string, data io.Reader) (*domain.ImportResult, error) {
	...
	doc, err := s.documents.Upload(ctx, companyID, expense.ID, filename, contentType, data)
	if err != nil {
		// Rollback: delete the skeleton expense.
		if delErr := s.expenses.Delete(ctx, companyID, expense.ID); delErr != nil {
	...
	if s.ocr != nil {
		ocrResult, ocrErr := s.ocr.ProcessDocument(ctx, companyID, doc.ID)
		if ocrErr != nil {
